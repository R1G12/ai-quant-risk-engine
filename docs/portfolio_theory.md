...
**Blended μ and tilt:** `sentiment_mu_blend` mixes historical and FinBERT expected returns; HMM `regime_sentiment_mix` scales effective α; `sentiment_magnitude_tilt` tilts gross weights after optimization. Implementation: `src/portfolio/expected_returns.py`, `regime_policy.py`, `sentiment_tilt.py`. See [run_profile.md — Sentiment in optimization](run_profile.md#sentiment-in-optimization).
